...
export default function useBlockchainActions() {
    ...
    return useMemo(() => ({
        ...
        withdrawFundsFromAirdrop: withdrawFundsFromAirdrop.bind(null, tonClient, sender),
    }), [sender]);
}
 ...
 * @param endTime
 */
async function createAirdrop(sender: Sender, provider: TonClient, {
    jettonAddress,
    entries,
    startTime,
    endTime
 ...
 */
async function sendJettonsToAirdrop(provider: TonClient, sender: Sender, airdropAddress: Address, amount: bigint, tokenMaster: Address) {
    ...
    if (!admin.equals(sender.address!)) throw new Error("You are not an admin of this airdrop");
    ...
    if (!(await masterWallet.getGetWalletAddress(airdropAddress)).equals(jettonWallet)) {
        throw new Error("Invalid jetton wallet address");
    }
    const ownerWallet = provider.open(JettonDefaultWallet.fromAddress(await masterWallet.getGetWalletAddress(sender.address!)));
    ownerWallet.send(sender, {
        value: toNano('0.1'),
        bounce: true,
    }, {
        $$type: 'TokenTransfer',
 ...
 * @param entries
 */
async function claimAirdrop(provider: TonClient, sender: Sender, {airdropAddress, entries}: ClaimAirdropParams) {
    ...
    if (merkleRoot !== merkleRootShouldBe) throw new Error("This is not a valid airdrop");
 ...
 * @param airdropAddress
 */
async function getAirdropData(provider: TonClient, airdropAddress: Address): Promise<AirdropData> {
    const airdrop = provider.open(Airdrop.createFromAddress(airdropAddress));
    const {admin, end, jettonWallet, begin} = await airdrop.getContractData();
    const {master} = await provider.open(JettonDefaultWallet.fromAddress(jettonWallet)).getGetWalletData();
    return {
        startTime: begin,
        endTime: end,
        admin,
 ...
 * @param amount
 */
async function withdrawFundsFromAirdrop(provider: TonClient, sender: Sender, airdropAddress: Address, amount: bigint) {
    const airdrop = provider.open(Airdrop.createFromAddress(airdropAddress));

    const {admin, end, begin} = await airdrop.getContractData();
    if (!admin.equals(sender.address!)) throw new Error("You are not an admin of this airdrop");
    const now = Date.now() / 1000;
    if (now < end && now > begin) throw new Error("Airdrop is active right now. You can withdraw funds only after it ends");
 ...
 */
async function isUserClaimedAirdrop(provider: TonClient, sender: Sender, airdropAddress: Address, airdropWallets: AirdropEntry[]): Promise<boolean> {
    const dict = generateEntriesDictionary(airdropWallets);
    const index = airdropWallets.findIndex(e => e.address.equals(sender.address!));
    if (index === -1) throw new Error("You are not in the airdrop list");
    const airdrop = provider.open(Airdrop.createFromAddress(airdropAddress));
    const {helperCode} = await airdrop.getContractData();
    const proof = dict.generateMerkleProof([BigInt(index)]);
    const helper = provider.open(AirdropHelper.createFromConfig({
        airdrop: airdropAddress,
        index: BigInt(index),
        proofHash: proof.hash()
    }, helperCode));
